...
} from "@moodlehq/ionic-native-push/ngx"
...
import {
  AsyncInstance,
  asyncInstance,
} from "@/core/utils/async-instance"
...
export class CorePushNotificationsProvider {
  ...
  async enableAnalytics(enable: boolean): Promise<void> {
    ...
    if (
      ...
      !win.PushNotification?.enableAnalytics
    ) {
      return
    }
    ...
    await new Promise<void>((resolve) => {
      win.PushNotification.enableAnalytics(
        ...
        (error) => {
          this.logger.error(
            "Error enabling or disabling Firebase analytics",
            enable,
            error
          )
  ...
  async logEvent(
    ...
    if (
      !CoreConstants.CONFIG.enableanalytics ||
      !win.PushNotification?.logEvent
    ) {
      return
    }

    // Check if the analytics is enabled by the user.
    const enabled = await CoreConfig.get<boolean>(
      CoreConstants.SETTINGS_ANALYTICS_ENABLED,
      true
    )
    if (!enabled) {
      return
  ...
  async onMessageReceived(
    ...
    const data: CorePushNotificationsNotificationBasicData =
      Object.assign(rawData, {
        title: notification.title,
        message: notification.message,
        customdata:
          typeof rawData.customdata == "string"
            ? CoreTextUtils.parseJSON<
    ...
    if (
      extraFeatures &&
      isAndroid &&
      CoreUtils.isFalseOrZero(data.notif)
    ) {
  ...
  async unregisterDeviceOnMoodle(site: CoreSite): Promise<void> {
    ...
    this.logger.debug(
      `Unregister device on Moodle: '${site.getId()}'`
    )
  ...
  async updateAddonCounter(
    ...
    if (
      !CorePushNotificationsDelegate.isCounterHandlerRegistered(addon)
    ) {
      return 0
    }
  ...
  async registerDeviceOnMoodle(
    ...
    try {
      ...
      if (neededActions.register) {
        ...
        // Device already registered, make sure the public key is up to date.
        const response = await this.updatePublicKeyOnMoodle(
          site,
          data
        )
        ...
        if (
          response?.warnings?.find(
            (warning) => warning.warningcode === "devicedoesnotexist"
          )
        ) {
  ...
  protected async getPublicKeyForSite(
    ...
    if (
      !site.wsAvailable("core_user_update_user_device_public_key")
    ) {
      return
    }
  ...
  protected async updatePublicKeyOnMoodle(
    ...
    CoreUserUpdateUserDevicePublicKeyWSResponse | undefined
  > {
    if (!data.publickey) {
      return
    }
    ...
    await Promise.all(
      results.map(async (result) => {
        ...
        const tmpSite = CoreSitesFactory.makeSite(
          ...
          result.siteurl,
          result.token,
          CoreTextUtils.parseJSON<CoreSiteInfo | null>(
            result.info,
            null
          ) || undefined
        )
 ...
 */
export type CoreUserAddUserDeviceWSResponse =
